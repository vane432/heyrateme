...
export default function UploadPost({ userId, onSuccess }: UploadPostProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2.5. Gender validation for all fashion posts
      ...
      await createPost(
        ...
        duration,
        mediaFile.size,
        null, // occasion (keeping for backward compatibility)
        gender
      );
    ...
    <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-xs text-gray-500 mt-1">
          What type of fashion look is this?
        </p>
      </div>
      ...
      <div className="mb-6">
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Gender Style <span className="text-red-500">*</span>
        </label>
        <select
          value={gender || ''}
        ...
        <p className="text-xs text-gray-500 mt-1">
          Help others find looks that match their style preferences
        </p>
      </div>
